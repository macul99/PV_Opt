...
class EnvConfig:
    ...
    kernal_l = 3
    kernal_w = 2
    max_reward = 20 # terminate if reward > max_reward
    max_steps = 100
...
def prepare_input_data_fixed(cfg):
    import pickle
    with open('/home/macul/Projects/hf_rl/gym_examples/gym_examples/envs/env_36x36_data.pkl','rb') as f:
        d_in, d_out, d_max = pickle.load(f)
    return d_in, d_out, d_max

class PanelGridWorldEnv_V1(gym.Env):
    '''
    Fixed env after reset, no random obstruction and land irr
    '''
    ...
    def __init__(self):
        ...
        self.data_raw, self.data_conv, self.data_max_irr = prepare_input_data_fixed(self.cfg)
    ...
    def reset(self, seed=None, options=None):
        ...
        if self._penalty_obstruction < 0:
            self._reward_panel_installation = 0
        else:
    ...
    def step(self, action):
        ...
        reward2 = penalty_obstruction - self._penalty_obstruction
        if reward2 == 0: # check panel innstallation reward
            reward_panel_installation = self._panel_installation_reward() # positive
        else:
            reward_panel_installation = self._reward_panel_installation
        reward1 = reward_panel_installation - self._reward_panel_installation
        ...
        if reward == 0:
            reward += -1 # edge of land, no installation reward, no obstruction penalty
